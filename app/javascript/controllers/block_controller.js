...
export default class extends Controller {
  ...
  connect() {
    ...
    console.log('Block controller connected')
  }

  toggleSummary(event) {
    event.preventDefault()
    const blockNumber = event.currentTarget.dataset.blockNumber
    const blockContainer = document.getElementById(`block_${blockNumber}`)
    
    if (!blockContainer) return
    
    // Show loading state
    this.showLoading(blockContainer)
    
    // Fetch summary via Turbo
    fetch(`/app/ethereum/blocks/summary?block_number=${blockNumber}`, {
      method: 'GET',
      headers: {
        'Accept': 'text/html',
    ...
    .then(html => {
      blockContainer.outerHTML = html
      // Re-initialize Stimulus controllers on the new content
      const newElement = document.getElementById(`block_summary_${blockNumber}`)
    ...
    .catch(error => {
      ...
      this.hideLoading(blockContainer)
    })
  }
  ...
  toggleDetail(event) {
    event.preventDefault()
    const blockNumber = event.currentTarget.dataset.blockNumber
    const summaryContainer = document.getElementById(`block_summary_${blockNumber}`)
    ...
    fetch(`/app/ethereum/blocks/detail?block_number=${blockNumber}`, {
      method: 'GET',
      headers: {
        'Accept': 'text/html',
        'X-Requested-With': 'XMLHttpRequest'
    ...
    .then(html => {
      ...
      // Re-initialize Stimulus controllers on the new content
      const newElement = document.getElementById(`block_${blockNumber}`)
      if (newElement && this.application) {
        this.application.start()
      }
    })
